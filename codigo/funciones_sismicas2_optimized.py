...
def generar_comentario2(ciudades, lat, lon, file_provincias):
    files = os.listdir(file_provincias)
    prov_pol = None
    prov_pol = de_que_provincia_es( lon,lat,files,file_provincias)

    if prov_pol is None:
        return "Distante."

    v = calcular_ciudad(ciudades, prov_pol, lat, lon)
    ...
    # Calcular el ángulo y ajustarlo a 360 grados
    angulo = math.degrees(math.atan2(dlat, dlon))
    if angulo < 0:
        angulo += 360

    # Calcular el índice de la dirección
    indice = int(round(angulo / 22.5)) % 16
    ...
    prov_pol_corregido = correcciones.get(prov_pol, prov_pol)

    comentario = f"{v[2]} Km al {direccion} de {v[3]}, {prov_pol_corregido}."
    return comentario
...
def formatear(select,ciudades,path_provincias):
    ...
    salida = f'{fecha}  {hora}  {lat}  {lon}  {prof}  {eprof}  {gap}  {rms}  {ml}  {mc}  {mw}  '
    comentario = generar_comentario2(ciudades,float(lat),float(lon),path_provincias)
...
def formatear_hyp(path_file, path_poligonos,path_ciudades,sentido,magni=1, gapLines='',focalLines=''):
    ...
    comentario = generar_comentario2(path_ciudades,float(lat),float(lon),path_poligonos)
...
def formatear_dummy(linea,ciudades,path_provincias):
    ...
    comment = generar_comentario2(ciudades,float(lat),float(lon),path_provincias)
...
def insertar_comentario(paths,formato,sentido):
    ...
    base = paths[6][:-1]
    ruta_hyp = paths[0][:-1]
    lineas_hyp = open(os.path.join(ruta_hyp)).readlines()
